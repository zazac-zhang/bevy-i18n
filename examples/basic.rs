...
fn setup(
    ...
    // Spawn translated text — the `I18nMarker` component resolves the key automatically
    commands.spawn((
        Text::new(""),
    ...
    commands.spawn((
        Text::new(""),
        I18nMarker::with_vars("player.greeting", &[("name", "Player")]),
    ));

    // Plural forms based on count
    commands.spawn((
        Text::new(""),
        I18nMarker::plural("player.inventory", 0),
    ));
}
